...
namespace perc
{
    ...
    struct Poller::CheshireCat
    {
        ...
        std::vector<Handle> mHandlesMutex;
        Handle mAddRemoveDoneEvent;

        std::mutex mWaitForMultipleObjectsMutex;
        Handle mWaitForMultipleObjectsEvent;
    };
    ...
    {
        mData->mAddRemoveDoneEvent = std::make_shared<std::condition_variable>();
    ...
    bool Poller::add(const Poller::event &evt)
    {
        ...
        mData->mAddRemoveDoneEvent->notify_all();
    ...
    void Poller::handleThread(Handle handle, std::mutex handleMutex)
    {
        while (true)
        {
            std::unique_lock<std::mutex> lk(handleMutex);
            handle->wait(lk, [] {return true; });
            mData->mWaitForMultipleObjectsEvent->notify_all();
        }
    }
    ...
    int Poller::poll(Poller::event &evt, unsigned long timeoutMs)
    {
        ...
        while (true)
        {
            int exitReason;
            {
                ...
                std::unique_lock<std::mutex> lk(mData->mWaitForMultipleObjectsMutex);
                auto now = std::chrono::system_clock::now();
                if (mData->mWaitForMultipleObjectsEvent->wait_until(lk, now) == std::cv_status::timeout)
                {
                    return 0;
                }
                ...
                if (eventIndex != 0)  // zero is special case - it is our private event
                {
